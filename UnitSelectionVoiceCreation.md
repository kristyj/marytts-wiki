...
**PraatPitchmarker**  
It computes pitch markers with help of Praat, phonetic analysis software. You need to compile or install Praat in your machine using:
    `sudo apt-get install praat`
It also does corrections for Pitch Marks to align near by Zero Crossing.  
...
**2 MCEPMaker**  
It calculate MFCCs from Speech Wave files, using Edinburgh Speech Tools. MFCC stands for Mel-Frequency Cepstral Coefficient, and aims to represent speech in a vector form that includes information about how humans perceive speech, rather than the absolute values of the original recording. 
...
**AllophonesExtractor** 
The way a word is written is not always how it is said, and this step bridges the gap between the text that the speaker read out, and the sounds that the speaker produced. The match between the words and the sounds is found in a pronunciation dictionary (in English this only supports the expected North American pronunciation), or generated by rules if the word is missing from the list.
This step analyses the text representations of the prompts from the txt files, and outputs an XML file for each utterance, breaking the words up into their constituent sounds using a pronunciation dictionary. It also marks where syllable breaks occur and which syllables are stressed (spoken louder than the other syllables in a word), as well as some pitch information.
This step creates the prompt_allophones directory required in the next step. `Note: This component requires the MARY server`

**EHMMLabeler**  
EHMM Labeler is a labeling tool, which generates label files with help of Wave files and corresponding Transcriptions.

These label files match the correct time period in the recording to the sound produced. It is not simple for a computer to understand which sound we make at any time by looking only at the waveform alone, so the method used for this step is forced alignment, where we presume that the speaker said each and every sound that is listed in the XML file (from the previous step). The program makes an estimation of which part of the recording corresponds to each phoneme, and iterates until it finds the best fit. This process leads to problems if the speaker said extra words or sounds, or skipped others.

EHMM basic tool is available with Festvox Recent Version. For running EHMM Labeler under MARY environment you need to compile EHMM tool in your machine. It may take long time depending on the size of the data and system configuration.  
...
Using the default list is recommended.
...
**CARTBuilder**  
It builds a preselection tree for the target cost features using "wagon" (CART) from the Edinburgh Speech tools. This is a "Classification and Regression Tree".
